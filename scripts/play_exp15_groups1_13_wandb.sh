...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/play_exp15_groups1_13_wandb.sh [--gpu N] [--num-envs N] \
    [--video-length N] [--viewer auto|native|viser] <group_number> <wandb_run_path>

Valid group_number values: 1..13
EOF
}
...
if ! [[ "$GROUP" =~ ^([1-9]|1[0-3])$ ]]; then
  echo "[ERROR] group_number must be an integer from 1 to 13. Got: $GROUP" >&2
  usage >&2
  exit 1
fi

MUJOCO_GL=egl PYOPENGL_PLATFORM=egl CUDA_VISIBLE_DEVICES="$PLAY_GPU" \
  uv run scripts/play.py "Smp-BodyVelocity-Exp15-Group${GROUP}-G1" \
