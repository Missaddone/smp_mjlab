...
Use `scripts/experiment_launcher.py` for Exp11, Exp12, Exp13, and Exp14 policy jobs. The shared root-level [`wandb_run_registry.csv`](wandb_run_registry.csv) is the canonical mapping across servers; it contains only `exp`, `group`, and the bare W&B `run_id`.
...
uv run scripts/experiment_launcher.py train exp14 9 --gpu 2
```
...
uv run scripts/experiment_launcher.py play exp14 1 --gpu 0 --viewer viser
```
